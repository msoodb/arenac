...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		case COLOR_LONG_OPT:
			if (optarg)
				command.color = optarg;
			break;
		case OUTPUT_LONG_OPT:
			if (optarg)
				command.output = optarg;
			break;
		case 'h':
			help(EXIT_SUCCESS);
			break;
	...
	if (file_exists(command.SOURCE) != 1) {
		printf("%s%s%s\n", "cannot stat '",
...
void init_command(void)
{
	...
	command.color = (char *)"RED";
	command.output = (char *)"json";
	command.SOURCE = NULL;
	command.PORT = -1;
}
